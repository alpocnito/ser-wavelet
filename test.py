...
def test(model: nn.Module, dataloader: DataLoader, device: torch.device) -> Dict:
    """
    ...
    if os.path.exists(params["output_path"]):
        ...
        if not k_fold is None:
            for fold in range(k_fold):
                ...
                scores = test(model=model, dataloader=test_dataloader, device=device)
